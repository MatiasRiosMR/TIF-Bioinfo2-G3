...
DB="/home/matias/probando_blast/db/swissprot"
...
echo -e "qseqid\tsseqid\tpident\tlength\tevalue\tbitscore\ttitle" > "$OUTPUT"
...
# Ejecución de BLASTp en segundo plano (Redirección de stderr al log)
blastp \
  -query "$INPUT" \
  -db "$DB" \
  -num_threads "$THREADS" \
  ...
  -outfmt "6 qseqid sseqid pident length evalue bitscore stitle" >> "$OUTPUT" 2>> "$LOG" &
